            ...
            <ContentContainer>
                {notes.map((note, index) =>
                    (note.title.includes(search) ||
                        ...
                        note.title.includes(search.toUpperCase()) ||
                        note.title.includes(search.charAt(0).toUpperCase() + search.slice(1)))
                        ? <NotePreviewCard key={index} cardId={index} openModal={openModal} noteBody={note.body} noteTitle={note.title} noteExtention={note.extention} noteId={note._id} user={user} />
                        : <></> )
                }
            </ContentContainer>
